...
Feature: FOB-EXPERTS-LIST+FIND-1 Experts Leaderboard
  ...
  Scenario: EXPERTS-LIST-03 Development tab shows top experts by primary category
    ...
    Then the leaderboard shows at most 5 rows
    And each row has primary category badge "Development"
    And rows are ordered by total Karma descending:
      | rank | display_name |
      |    1 | Mike Chen    |
      |    2 | Jordan Lee   |

  Scenario: EXPERTS-LIST-04 Each row shows rank, avatar, name, Karma, badge, and hint
    Given Elena is on /experts/?category=development
    Then the row for "Mike Chen" (data-testid="experts-row-mchen") displays:
      | column           | value                                              |
      | Rank             | 1                                                  |
  ...
  Scenario: EXPERTS-LIST-05 View profile links to public expert detail
    Given Elena is on /experts/?category=development
    When she clicks [View profile] on the row for "Mike Chen"
  ...
  Scenario: EXPERTS-LIST-06 Empty category tab shows encouragement message
    ...
    And data-testid="experts-leaderboard" shows zero rows
