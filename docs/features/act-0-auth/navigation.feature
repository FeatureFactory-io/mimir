Feature: FOB-DASHBOARD-1 Dashboard and Navigation
  ...
  Scenario: FOB-DASHBOARD-01 View dashboard overview
    ...
    And she sees "Recently Contributed" section showing last 10 accessed items (Playbooks/Workflows/Activities) with usage counts
    And she sees quick action buttons for Create/Import/Sync

  Scenario: FOB-DASHBOARD-02 Recently Contributed section shows usage statistics
    Given Maria is on the dashboard
  ...
  Scenario: FOB-DASHBOARD-03 Recently Contributed tracks only views, not edits
    Given Maria is on the dashboard
    When she views the "Recently Contributed" section
  ...
  Scenario: FOB-DASHBOARD-09 Default Recently Contributed feed shows last 24h only
    Given Maria is on the dashboard
    Then she sees the "Recently Contributed" section with data-testid "recently-used-section"
    And the time filter shows [Last 24h] with data-testid "recently-used-hours-label"
    And every activity row in the Recently Contributed feed has a timestamp within the last 24 hours
    And she does not see activity rows older than 24 hours

  Scenario: FOB-DASHBOARD-10 Recently Contributed Last hour filter via HTMX
    Given Maria is on the dashboard
    ...
    When she selects [Last hour] in the Recently Contributed time filter
  ...
  Scenario: FOB-DASHBOARD-11 Recently Contributed Last week filter includes and excludes by age
    Given Maria is on the dashboard
    And she has an activity accessed 3 days ago
    And she has an activity accessed 10 days ago
    When she selects [Last week] in the Recently Contributed time filter
    Then the feed shows the activity from 3 days ago
    And the feed does not show the activity from 10 days ago

  Scenario: FOB-DASHBOARD-12 Recently Contributed Refresh preserves selected time window
    Given Maria is on the dashboard
    When she selects [Last hour] in the Recently Contributed time filter
    And she clicks [Refresh] in the Recently Contributed section
    Then the refresh request includes "?hours=1"
    And the time filter still shows [Last hour]

  Scenario: FOB-DASHBOARD-13 Recently Contributed badge shows count in selected window
    Given Maria is on the dashboard
