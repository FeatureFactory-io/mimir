...
Feature: FOB-COPY-PROMPT-VIEW-1 Copy Prompt on entity detail pages
  ...
  Scenario: FOB-COPY-PROMPT-VIEW-07 PIP detail header includes Copy Prompt
    Given Maria is on FOB-PIP-DETAIL-1 for PIP-42
    Then she sees data-testid "copy-prompt-btn" in data-testid "pip-detail-actions"

  Scenario: FOB-COPY-PROMPT-VIEW-08 Prompt text uses hidden textarea not data-copy attribute
  ...
  Scenario: FOB-COPY-PROMPT-VIEW-09 Clipboard denial shows non-blocking error
  ...
  Scenario: FOB-COPY-PROMPT-VIEW-10 Activity embed mode includes Copy Prompt
  ...
  Scenario: FOB-COPY-PROMPT-VIEW-11 Content Browser detail panel embed includes Copy Prompt
